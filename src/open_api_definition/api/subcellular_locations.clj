(ns open-api-definition.api.subcellular-locations
...
(defn-spec get-by-id1-with-http-info any?
  "Get subcellular location entry by a single accession.
  Search subcellular location entry by id to return all data associated with that entry. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([id string?, ] (get-by-id1-with-http-info id nil))
...
(defn-spec get-by-id1 subcellular-location-entry-spec
  "Get subcellular location entry by a single accession.
  Search subcellular location entry by id to return all data associated with that entry. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([id string?, ] (get-by-id1 id nil))
  ([id string?, optional-params any?]
   (let [res (:data (get-by-id1-with-http-info id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode subcellular-location-entry-spec res st/string-transformer)
        res))))


(defn-spec search1-with-http-info any?
...
(defn-spec search1 search-result-spec
  "Retrieve subcellular location entries by a search query.
  The search endpoint uses a request query to return all entries associated with the search term in a paginated list of entries. Use ‘size’ to specify the number of entries per page of results. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([query string?, ] (search1 query nil))
  ([query string?, optional-params any?]
   (let [res (:data (search1-with-http-info query optional-params))]
     (if (:decode-models *api-context*)
        (st/decode search-result-spec res st/string-transformer)
        res))))


(defn-spec stream1-with-http-info any?
  ...
  ([query string?, ] (stream1-with-http-info query nil))
  ([query string?, {:keys [sort fields download]} (s/map-of keyword? any?)]
   (check-required-params query)
   (call-api "/locations/stream" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec stream1 stream-result-spec
  "Download subcellular location entries retrieved by a search query.
  The stream endpoint uses a request query to return all entries associated with the search term in a single download. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([query string?, ] (stream1 query nil))
  ([query string?, optional-params any?]
   (let [res (:data (stream1-with-http-info query optional-params))]
     (if (:decode-models *api-context*)
