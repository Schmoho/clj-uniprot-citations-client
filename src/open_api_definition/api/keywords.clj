(ns open-api-definition.api.keywords
...
(defn-spec get-by-id2-with-http-info any?
  "Get keyword entry by a single accession.
  Search keyword entry by id to return all data associated with that entry. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([id string?, ] (get-by-id2-with-http-info id nil))
  ([id string?, {:keys [fields]} (s/map-of keyword? any?)]
   (check-required-params id)
   (call-api "/keywords/{id}" :get
             {:path-params   {"id" id }
              :header-params {}
              :query-params  {"fields" fields }
              :form-params   {}
              :content-types []
              :accepts       ["application/json" "text/plain;format=tsv" "text/plain;format=list" "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" "application/rdf+xml" "text/plain;format=obo"]
              :auth-names    []})))

(defn-spec get-by-id2 keyword-entry-spec
  "Get keyword entry by a single accession.
  Search keyword entry by id to return all data associated with that entry. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([id string?, ] (get-by-id2 id nil))
  ([id string?, optional-params any?]
   (let [res (:data (get-by-id2-with-http-info id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode keyword-entry-spec res st/string-transformer)
        res))))


(defn-spec search2-with-http-info any?
...
(defn-spec search2 search-result-spec
  "Retrieve keyword entries by a search query.
  The search endpoint uses a request query to return all entries associated with the search term in a paginated list of entries. Use ‘size’ to specify the number of entries per page of results. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([query string?, ] (search2 query nil))
  ([query string?, optional-params any?]
   (let [res (:data (search2-with-http-info query optional-params))]
     (if (:decode-models *api-context*)
        (st/decode search-result-spec res st/string-transformer)
        res))))


(defn-spec stream2-with-http-info any?
  ...
  ([query string?, ] (stream2-with-http-info query nil))
  ([query string?, {:keys [sort fields download]} (s/map-of keyword? any?)]
   (check-required-params query)
   (call-api "/keywords/stream" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec stream2 stream-result-spec
  "Download keyword entries retrieved by a search query.
  The stream endpoint uses a request query to return all entries associated with the search term in a single download. Specify <tt>fields</tt> to return only data for specific sections of that entry that are of interest to you"
  ([query string?, ] (stream2 query nil))
  ([query string?, optional-params any?]
   (let [res (:data (stream2-with-http-info query optional-params))]
     (if (:decode-models *api-context*)
